...
namespace SpaceCalc_PRO
{
    public partial class frmVisaoProduto : Form
    {
        ...
        {
            frmCadastroProduto frm = new frmCadastroProduto(null, false);
            frm.ShowDialog();
            atualizaGrid();
        }
        ...
        private async void btnRemoverProduto_Click(object sender, EventArgs e)
        {
            try
            {
                if (kryptonDataGridView1.SelectedRows.Count > 0)
                {
                    ...
                    {
                        bool deleteResult = await MongoDB.deleteDT("ZPRODUTO", id);
        ...
        private void btnEditar_Click(object sender, EventArgs e)
        {
            if (kryptonDataGridView1.SelectedRows.Count > 0)
            {
                ...
                frmCadastroProduto frm = new frmCadastroProduto(id, true);
                frm.ShowDialog();
                atualizaGrid();
            }
            else
